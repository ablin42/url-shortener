...
import { BrowserRouter as Router, Switch, Route, useRouteMatch, useParams, Redirect } from "react-router-dom";
import "./index.css";
...
function Toggler() {
	...
	useEffect(() => {
		...
		let checkbox = document.querySelector("input[name=theme]") as HTMLInputElement;
	...
	return (
		<>
			...
			<div className="form-wrapper">
				<form method="POST" action="/" onSubmit={handleSubmit} className="shorten-form">
					<input
...
//export const Card = ({ title, paragraph }: CardProps) => <aside></aside>
...
function Shortened({ id, longUrl, shortUrl }: CardProps) {
	...
	return (
		<div className="url-container">
			...
			<a target="_blank" href={longUrl} rel="noopener noreferrer" className="shortened-link heroAnim">
				xs-url.fr/{id}
			</a>
...
function ShortenResult() {
	...
	console.log(state.id, state.longUrl, state.shortUrl, "x");
	if (state.id.length > 0) {
		console.log(state.id, state.longUrl, state.shortUrl, "z");
		return (
			<Switch>
				<Route exact path={`${match.url}/:id`}>
					<Shortened id={state.id} longUrl={state.longUrl} shortUrl={state.shortUrl} />
				</Route>
